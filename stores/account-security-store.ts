...
export const useAccountSecurityStore = create<AccountSecurityState>()((set, get) => ({
  ...
  probe: async () => {
    set({ isProbing: true });
    try {
      const response = await fetch(apiPath('/api/account/stalwart/probe'), {
  ...
  fetchAuthInfo: async () => {
    ...
    try {
      const response = await fetch(apiPath('/api/account/stalwart/auth'), {
        headers: getApiHeaders(),
      });
  ...
  fetchCryptoInfo: async () => {
    ...
    try {
      const response = await fetch(apiPath('/api/account/stalwart/crypto'), {
        headers: getApiHeaders(),
      });
      if (!response.ok) throw new Error(`HTTP ${response.status}`);
      const data = await response.json();
  ...
  fetchPrincipal: async () => {
    ...
    try {
      const response = await fetch(apiPath('/api/account/stalwart/principal'), {
        headers: getApiHeaders(),
      });
      if (!response.ok) {
